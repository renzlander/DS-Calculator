...
export const handleSubmit = (
  ...
  if (algorithm == "fifo") {
    ...
    for (let i = 1; i < sortedRequests.length; i++) {
      if (
        (sortedRequests[i] > previousHeadPosition &&
          sortedRequests[i + 1] < previousHeadPosition) ||
        (sortedRequests[i] < previousHeadPosition &&
          sortedRequests[i + 1] > previousHeadPosition)
      ) {
        insertIndex = i + 1;
    ...
    for (let i = 1; i < arrangedRequests.length; i++) {
      if (
        (arrangedRequests[i] > previousHeadPosition &&
          arrangedRequests[i + 1] < previousHeadPosition) ||
        (arrangedRequests[i] < previousHeadPosition &&
          arrangedRequests[i + 1] > previousHeadPosition)
      ) {
        insertIndex = i + 1;
